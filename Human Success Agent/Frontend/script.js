...
function updatePhaseUI() {
    ...
    const day = getProtocolDay();
    dayCounter.textContent = `Day ${day} of 21`;
}

// Day counter based on real calendar days since first use (Option A).
// A new day ticks over at local midnight; stays on Day 1 no matter how
// many messages are sent in a single sitting. Capped at Day 21.
function getProtocolDay() {
    const now = new Date();
    const todayMidnight = new Date(now.getFullYear(), now.getMonth(), now.getDate());

    let startStr = localStorage.getItem('protocol_start_date');
    if (!startStr) {
        // First time using it — today becomes Day 1.
        localStorage.setItem('protocol_start_date', todayMidnight.toISOString());
        startStr = todayMidnight.toISOString();
    }

    const start = new Date(startStr);
    const startMidnight = new Date(start.getFullYear(), start.getMonth(), start.getDate());

    const dayDiff = Math.floor((todayMidnight - startMidnight) / 86400000);
    return Math.min(Math.max(dayDiff + 1, 1), 21);
}
...
newJourneyBtn.addEventListener('click', async () => {
    if (confirm('Start a new journey? This will clear your conversation history.')) {
        ...
        localStorage.removeItem('protocol_start_date'); // restart Day 1 for the new journey
